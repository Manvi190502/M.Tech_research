...
class FeatureEnhancementGatingModule(nn.Module):
    ...
    def forward(self, x):
        ...
        # 1. Channel Attention
        # -------------------------
        x_pool = torch.mean(x, dim=(2, 3))     # [B, C]
        x_attn = self.fc1(x_pool)
        x_attn = self.relu(x_attn)
        x_attn = self.fc2(x_attn)
        x_attn = torch.sigmoid(x_attn).view(B, C, 1, 1)
...
class MixVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.apply(self._init_weights)
        # 🔥 Multi-scale enhancement modules (lightweight)
        self.enhance1 = FeatureEnhancementGatingModule(dim=embed_dims[0])
        self.enhance2 = FeatureEnhancementGatingModule(dim=embed_dims[1])
        self.enhance3 = FeatureEnhancementGatingModule(dim=embed_dims[2])
        self.enhance4 = FeatureEnhancementGatingModule(dim=embed_dims[3])
    ...
    def forward_features(self, x):
        ...
        x = x + self.enhance1(x)   # residual enhancement
        outs.append(x)
        ...
        x = self.norm2(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x = x + self.enhance2(x)   # residual enhancement
        outs.append(x)
        # stage 3
        ...
        x = self.norm3(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x = x + self.enhance3(x)   # residual enhancement
        outs.append(x)
        ...
        x = self.norm4(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x = x + self.enhance4(x)   # residual enhancement
        outs.append(x)
...
class mit_b0_1(MixVisionTransformer):
    def __init__(self, **kwargs):
        super(mit_b0_1, self).__init__(
